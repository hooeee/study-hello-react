...
import { addCount, addItem } from "../../../store/index.js";
    ...
    <div className={"container start-opacity " + fadeOpacity}>
      <div className="div-main">
        <div className="container">
          {view == false ? null : (
            <div className="alert alert-caution">경고: 숫자만입력하세요</div>
          )}
          {hidden == true ? (
          ...
          <div className="row">
            ...
            <div className="col-md-6">
              ...
              <button
                ...
                onClick={() => window.open(findID.link, "_blank")}
              >
                주문하기
              </button>
            </div>
  ...
  return (
    <div>
      <span>
        ...
        <p>
          시간당 1500원입니다. 대화 후 견적 산출 가능해요 모델링 파일 첨부
          해주시면 화면에 시간 확인 후 결제 및 출력 시작 합니다 현재는 모델링은
          진행하지 않습니다 . 출력물 무게에 따라 배송비 유무 있습니다 3d프린터
          속채움 및 나머지 설정은 현재 기본 상태로 출력 예정이며 혹여 원하시는
          조건 있으면 상의후 출력 해드려요 Fdm방식의 프린터로 약간의 오차는
          생기고 면에 줄은 생길수 있는데 원래 프린터 자체출력물에 생기는 것으로
          참고 바랍니다 가격협의 가능하고 최선을 다해서 출력해 보내드리겠습니다
          간단한 모델링 같은 경우 치수랑 알려주시면 만들어드려요(난이도에따라
          제작및 출력 2.5만원부터 시작합니다 하지만 협의에 의해 달라질수 있어요
          문의 많이 주세요) 가격 협의 가능하니 언제든지 톡 환영합니다 자세한
          문의 카카오톡 플러스아이디 검색 : dp3 문의 주셔도 감사하겠습니다.
        </p>
      </span>
    </div>
  );
...
function ReviewInfomation() {
  const reviewer = [
    {
      ...
      imgSrc: "./../review1.png",
      reviewWrite:
        "배송도 빠르고 잘나와서 매우 만족합니다. 감사합니다. 잘쓸게요~^^",
    },
    {
      username: "HiTomorrow",
      imgSrc: "./../review0.png",
      reviewWrite:
        "쿠키틀 주문제작이 너무 비싸서 3d 프린팅으로 주문했는데, 모양 수정도 뚝딱 해주시고 친절하게 상담해주셔서 믿고 두 번이나 맡겼습니다! 다음에도 또 부탁드릴게요☺️",
    },
    {
      username: "영앤뤼치",
      imgSrc: "./../review2.png",
      reviewWrite:
        "옛날에 구매했는데 이제야 후기를 다네요ㅜㅜ여기 사장님 진짜 최고십니다. 원하는 3d모형을 만들어주실려고 드라이기로 열까지 가열해주시고 결국 정말 제가 원하는 피규어 똑같이 나왔습니다.가격도 정말싸고 최고십니다!!  다음에 또 거래하고 싶습니다~",
    },
  ];
  ...
  return (
    <div>
      <div className="item-main">{ReviewList}</div>
    </div>
  );
}
...
function Review({ reviewer, i }) {
  return (
    <div className="item-sub">
      ...
      <div className="review-main">{reviewer[i].reviewWrite}</div>
    </div>
  );
}
